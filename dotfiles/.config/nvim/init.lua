...
vim.keymap.set("n", "<A-P>", '"+P', { desc = "Paste from system clipboard before cursor" })
...
vim.keymap.set("n", "<A-Y>", '"+yy', { desc = "Copy line to clipboard" })
...
vim.keymap.set("n", "<A-y>", '"+y', { desc = "Copy to clipboard" })
vim.keymap.set("n", "<A-z>", "<Cmd>FzfLua zoxide<CR>", { desc = "Switch to different directory" })
...
vim.keymap.set("n", "<Leader><space>", function() require("lazyvim.util").pick("files", {root = false})() end, { desc = "Find Files (cwd)" }) -- Override <Leader><Leader> to work in cwd.
