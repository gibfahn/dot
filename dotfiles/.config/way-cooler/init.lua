...
local keys = {
    -- Open dmenu
    key({mod}, "d", util.program.spawn_once("dmenu_run")), -- Open terminal
    ...
    key({mod}, "l", "dmenu_eval"), -- Move focus
    key({mod}, "left", "focus_left"), key({mod}, "right", "focus_right"),
    key({mod}, "up", "focus_up"), key({mod}, "down", "focus_down"),
    ...
    key({mod, "Shift"}, "down", "move_active_down"), -- Split containers
    key({mod}, "h", "split_horizontal"), key({mod}, "v", "split_vertical"),
    key({mod}, "e", "horizontal_vertical_switch"),
    key({mod}, "s", "tile_stacked"), key({mod}, "w", "tile_tabbed"),
...
for i = 1, 9 do
    table.insert(keys, key({mod}, tostring(i), "switch_workspace_" .. i))
    table.insert(keys,
                 key({mod, "Shift"}, tostring(i), "move_to_workspace_" .. i))
end

-- Register the keybindings.
for _, key in pairs(keys) do way_cooler.register_key(key) end
...
way_cooler.on_init = function() util.program.spawn_startup_programs() end

--- Execute some code when Way Cooler restarts
way_cooler.on_restart = function() util.program.restart_startup_programs() end

--- Execute some code when Way Cooler terminates
way_cooler.on_terminate =
